    ...
    return fetch(`${contextPath}/feedback?action=getEligibleBooking&carId=${carId}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                // Hiển thị thông báo lỗi nếu không có booking hợp lệ
                showNoEligibleBookingsError(data.message);
...
/**
 * Hiển thị thông báo lỗi khi không có booking hợp lệ để đánh giá
 * @param {string} message - Thông báo lỗi
 */
function showNoEligibleBookingsError(message) {
    // Tạo modal thông báo lỗi nếu chưa có
    if (!document.getElementById('noEligibleBookingsModal')) {
        const modalHtml = `
        <div class="modal fade" id="noEligibleBookingsModal" tabindex="-1" aria-labelledby="noEligibleBookingsModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="noEligibleBookingsModalLabel">Cannot Add Review</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <p id="noEligibleBookingsMessage">You have already reviewed all your completed bookings for this car.</p>
                    </div>
    ...
    document.getElementById('noEligibleBookingsMessage').textContent = message || 'You have already reviewed all your completed bookings for this car.';
    
    // Hiển thị modal
    const modal = new bootstrap.Modal(document.getElementById('noEligibleBookingsModal'));
    modal.show();
    
    // Ẩn form đánh giá mới nếu đang hiển thị
    const newReviewFormContainer = document.getElementById('newReviewFormContainer');
    const showNewReviewFormBtn = document.getElementById('showNewReviewFormBtn');
    if (newReviewFormContainer && showNewReviewFormBtn) {
        newReviewFormContainer.style.display = 'none';
        showNewReviewFormBtn.style.display = 'block';
    }
}
