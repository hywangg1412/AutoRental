...
public class BookingInsuranceService {
    ...
    public List<BookingInsurance> createInsurancesForBooking(Booking booking, int carSeats, boolean hasAdditionalInsurance) {
        ...
        try {
            LOGGER.info("=== BẮT ĐẦU TẠO BẢO HIỂM CHO BOOKING ===");
            LOGGER.info("Booking ID: " + booking.getBookingId());
            LOGGER.info("Thông tin đầu vào: carSeats=" + carSeats + ", hasAdditionalInsurance=" + hasAdditionalInsurance);
            LOGGER.info("Giá trị hasAdditionalInsurance: " + (hasAdditionalInsurance ? "TRUE" : "FALSE"));
            
            // Tính số ngày thuê để tính bảo hiểm
            double rentalDays = calculateRentalDaysForInsurance(booking);
            LOGGER.info("Số ngày thuê để tính bảo hiểm: " + rentalDays);
            
            // 1. Tạo bảo hiểm vật chất (BẮT BUỘC)
            LOGGER.info("1. Bắt đầu tạo bảo hiểm vật chất (bắt buộc)");
            ...
            if (vehicleInsurance != null) {
                ...
                LOGGER.info("Đã tạo bảo hiểm vật chất: " + vehicleInsurance.getPremiumAmount() + "K");
            } else {
                LOGGER.warning("Không thể tạo bảo hiểm vật chất cho booking: " + booking.getBookingId());
            }
            
            // 2. Tạo bảo hiểm tai nạn CHỈ KHI hasAdditionalInsurance=true
            if (hasAdditionalInsurance) {
                LOGGER.info("2. Bắt đầu tạo bảo hiểm tai nạn vì hasAdditionalInsurance=true");
                ...
                if (accidentInsurance != null) {
                    ...
                    LOGGER.info("Đã tạo bảo hiểm tai nạn: " + accidentInsurance.getPremiumAmount() + "K");
                } else {
                    LOGGER.warning("Không thể tạo bảo hiểm tai nạn cho booking: " + booking.getBookingId());
                }
            } else {
                LOGGER.info("2. Bỏ qua tạo bảo hiểm tai nạn vì hasAdditionalInsurance=false");
            }
            
            LOGGER.info("Hoàn thành tạo " + result.size() + " bảo hiểm cho booking");
            LOGGER.info("=== KẾT THÚC TẠO BẢO HIỂM CHO BOOKING ===");
    ...
    private BookingInsurance createAccidentInsurance(Booking booking, int carSeats, double rentalDays) {
        try {
            LOGGER.info("=== BẮT ĐẦU TẠO BẢO HIỂM TAI NẠN ===");
            LOGGER.info("Booking ID: " + booking.getBookingId());
            LOGGER.info("Số chỗ ngồi: " + carSeats);
            LOGGER.info("Số ngày thuê: " + rentalDays);
            ...
            LOGGER.info("Đã tìm thấy bảo hiểm tai nạn: " + accidentInsurance.getInsuranceName());
            LOGGER.info("InsuranceId: " + accidentInsurance.getInsuranceId());
            LOGGER.info("BaseRatePerDay: " + accidentInsurance.getBaseRatePerDay() + " VND");
            ...
            LOGGER.info("Tính toán phí bảo hiểm tai nạn:");
            LOGGER.info("- Phí bảo hiểm/ngày: " + premiumPerDay + " VND");
            LOGGER.info("- Số ngày thuê: " + rentalDays);
            LOGGER.info("- Tổng phí bảo hiểm: " + totalPremium + " VND (" + premiumAmountInK + "K)");
            ...
            // Lưu vào database
            LOGGER.info("Lưu bảo hiểm tai nạn vào database...");
            BookingInsurance savedInsurance = bookingInsuranceRepository.add(bookingInsurance);
            if (savedInsurance != null) {
                LOGGER.info("Đã lưu thành công bảo hiểm tai nạn với ID: " + savedInsurance.getBookingInsuranceId());
            } else {
                LOGGER.warning("Không thể lưu bảo hiểm tai nạn vào database");
            }
            
            LOGGER.info("=== KẾT THÚC TẠO BẢO HIỂM TAI NẠN ===");
    ...
    private Insurance findAccidentInsuranceBySeats(int carSeats) {
        try {
            LOGGER.info("=== TÌM BẢO HIỂM TAI NẠN PHÙ HỢP ===");
            LOGGER.info("Số chỗ ngồi cần tìm: " + carSeats);
            
            // Tìm bảo hiểm tai nạn với loại "TaiNan"
            List<Insurance> accidentInsurances = insuranceRepository.findByType("TaiNan");
            LOGGER.info("Tìm thấy " + accidentInsurances.size() + " bảo hiểm tai nạn trong database");
            
            if (accidentInsurances.isEmpty()) {
                LOGGER.warning("Không tìm thấy bảo hiểm tai nạn nào trong database");
                return null;
            }
            
            // Liệt kê tất cả các bảo hiểm tai nạn tìm thấy
            LOGGER.info("Danh sách bảo hiểm tai nạn tìm thấy:");
            for (int i = 0; i < accidentInsurances.size(); i++) {
                Insurance ins = accidentInsurances.get(i);
                LOGGER.info((i+1) + ". " + ins.getInsuranceName() + 
                        " (ApplicableCarSeats: " + ins.getApplicableCarSeats() + 
                        ", BaseRatePerDay: " + ins.getBaseRatePerDay() + " VND)");
            }
            
            // Lọc theo số chỗ ngồi
            LOGGER.info("Bắt đầu lọc theo số chỗ ngồi...");
            for (Insurance insurance : accidentInsurances) {
                String seatRange = insurance.getApplicableCarSeats();
                boolean isApplicable = isApplicableSeatRange(carSeats, seatRange);
                
                LOGGER.info("Kiểm tra " + insurance.getInsuranceName() + 
                        " (ApplicableCarSeats: " + seatRange + ") - " + 
                        (isApplicable ? "PHÙ HỢP" : "KHÔNG PHÙ HỢP"));
                
                if (isApplicable) {
                    LOGGER.info("Đã tìm thấy bảo hiểm phù hợp cho xe " + carSeats + " chỗ: " + 
    ...
    private boolean isApplicableSeatRange(int seats, String seatRange) {
        LOGGER.info("Kiểm tra phạm vi chỗ ngồi: seats=" + seats + ", seatRange=" + seatRange);
        
        if (seatRange == null) {
            LOGGER.info("seatRange=null, áp dụng cho mọi loại xe");
            return true;
        }
        
        boolean result = false;
        
        if (seatRange.equals("1-5")) {
            result = seats <= 5;
            LOGGER.info("Phạm vi 1-5 chỗ: " + (result ? "PHÙ HỢP" : "KHÔNG PHÙ HỢP"));
        } else if (seatRange.equals("6-11")) {
            result = seats >= 6 && seats <= 11;
            LOGGER.info("Phạm vi 6-11 chỗ: " + (result ? "PHÙ HỢP" : "KHÔNG PHÙ HỢP"));
        } else if (seatRange.equals("12+")) {
            result = seats >= 12;
            LOGGER.info("Phạm vi 12+ chỗ: " + (result ? "PHÙ HỢP" : "KHÔNG PHÙ HỢP"));
        } else {
            LOGGER.info("Phạm vi không xác định, áp dụng cho mọi loại xe");
            result = true;
        }
